...
$\left( 1 + \vartheta_{1}B + \ldots{+ \ \vartheta}_{q}B^{q} \right)\left( \ 1 + \Theta_{1}B^{s} + \ldots{+ \ \Theta}_{Q}B^{\text{sQ}} \right);$
...
The equation \[7.26\] can be expressed as:
  ...
  ------------------------------------------------------------------------------------------------------------------------------------ ----------
  ${\psi^{- 1}(B)x}_{t} = a_{t}(1 + \varphi_{1}B + \ldots\varphi_{p}B^{p})x_{t} = {(1 + \theta_{1}B + \ldots\theta_{q}B^{q})a}_{t}$,   \[7.32\]
  ------------------------------------------------------------------------------------------------------------------------------------ ----------
...
and the left hand side becomes:
  ...
  ---------------------------------------------------------------------------------------------------------- ----------
  ${\varphi(B)\gamma_{k} = \gamma}_{k} + \varphi_{1}\gamma_{k - 1} + \ldots\varphi_{p}\gamma_{k - p} = 0$,   \[7.34\]
  ---------------------------------------------------------------------------------------------------------- ----------
...
outside the unit circle). Therefore, from the equation
  ...
  --------------------------------------------------------------- ----------
  ${\varphi(B)}^{- 1} = \frac{1}{(1 - z_{1})\ldots(1 - z_{1})}$   \[7.37\]
  --------------------------------------------------------------- ----------
...
of$\text{\ x}_{t}$ into additive spectra of the components. When the AMB
...
identified from the model of $\text{\ x}_{t}$. As sketched above, to
...
$\text{\ ψ}_{i}\left( F \right) = \frac{\theta_{i}\left( F \right)}{\phi_{i}\left( F \right)}$
...
$x_{t}\text{\ \ }$is divided into the spectra of its components (in the
...
$\theta\left( B \right)a_{t} = \theta_{s}(B)a_{\text{st}}{+ \theta}_{n}(B)a_{\text{nt}}$.

As the time series components are never observed, their estimators have
to be used. Let us note $X_{T}\text{\ \ }$an infinite realization of the
...
the conditional expectation$\text{\ E}\left( s_{t}|X_{T} \right)$, so it
can be presented as a linear function of the elements
in$\text{\ X}_{T}$.[^50] WHITTLE (1963) shows that the MMSE estimator of
${\widehat{s}}_{t}$ is:
...
where$\text{\ ψ}\left( B \right) = \frac{\theta(B)}{\varphi(B)}$,
$F = B^{- 1}$ and $k_{s} = \frac{V(a_{s})\ }{V(a)\ }$,
$V\left( a_{s} \right)$ is the variance of $a_{\text{st}}$ and $V(a)$ is
...
for large $\text{k\ }$the estimator for the central periods of the
...
filter, and therefore contains an error $e_{\text{st}}$ called final
estimation error, which is equal $e_{\text{st}} =$
$s_{t} - {\widehat{s}}_{t}$.
...
available yet. For this reason a preliminary estimator need to be used.
...
at $T$ $(T - t = k \geq 0)$ can be expressed as
  ...
  -------------------------------------------------------------------- ----------
  ${\widehat{x}}_{it|t + k} = \nu_{i}\left( B,F \right)x_{t|T}^{e}$,   \[7.55\]
  -------------------------------------------------------------------- ----------
...
model on$\text{\ x}_{t}$. When $k = 0$ the preliminary estimator becomes
the concurrent estimator. As the forecasts are linear functions of
present and past observations of$\text{\ x}_{t}$, the preliminary
estimator ${\widehat{x}}_{\text{it}}$ will be a truncated asymmetric
filter applied to$\text{\ x}_{t}$ that generates a phase effect[^56].
...
final estimation error ($e_{\text{it}}$) and the revision error
($r_{it|t + k}$), i.e.:

  -- ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- ----------
     $d_{it|t + k} = x_{\text{it}} - {\widehat{x}}_{it|t + k} = \left( x_{\text{it}} - {\widehat{x}}_{\text{it}} \right) + \left( {\widehat{x}}_{\text{it}} - {\widehat{x}}_{it|t + k} \right) = e_{\text{it}} + r_{it|t + k}$,   \[7.56\]
  -- ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- ----------

where:

$x_{\text{it}}$ - $i^{th}\ $component;

${\widehat{x}}_{it|t + k}$- the estimator of $x_{\text{it}}\ $when the
last observation is $x_{t + k}$.

Therefore the preliminary estimator is subject not only to the final
error but also to a revision error, which are orthogonal each
other[^58]. The revision error decreases as $\text{k\ }$increases, until
...
${\widehat{x}}_{\text{it}} = \nu_{s}(B,F)x_{t}$. By replacing
$x_{\text{it}} = \frac{\theta(B)}{\gamma(B)\delta(B)}a_{t}$, the
component estimator can be expressed as[^59]:

  -------------------------------------------------- ----------
  ${\widehat{x}}_{\text{it}} = \xi_{s}(B,F)a_{t}$,   \[7.57\]
  -------------------------------------------------- ----------
...
innovation$\text{\ a}_{t}$, rather than on the
series$\text{\ x}_{t}$[^60]. Hence, the filter from \[7.56\] can be
divided into two components: the first one, i.e.
${\ldots + \xi}_{j}B^{j} + \ldots{+ \xi}_{1}B + \xi_{0}$, applies to
prior and concurrent innovations, the second one, i.e.
$\xi_{- 1}F + \ldots + \xi_{- j}F^{j}\ $applies to future (i.e.
posterior to$\text{\ t}$) innovations. Consequently, $\xi_{j}$
...
expressed as:
  ...
  ---------------------------------------------------------------------------------- ----------
  ${\widehat{x}}_{\text{it}} = \xi_{i}{(B)}^{-}a_{t} + \xi_{i}{(F)}^{+}a_{t + 1}$,   \[7.58\]
  ---------------------------------------------------------------------------------- ----------
...
**The Psi-weights for stable and stochastic seasonal components**
...
concurrent estimator is equal to
  ...
  --------------------------------------------------------------------------------------------------------- ----------
  ${\widehat{x}}_{it|t} = {E_{t}x_{\text{it}} = {E_{t}\widehat{x}}_{\text{it}} = \xi}_{i}{(B)}^{-}a_{t}$,   \[7.59\]
  --------------------------------------------------------------------------------------------------------- ----------

so that the revision

  ------------------------------------------------------------------------------------------------ ----------
  $r_{\text{it}} = {\widehat{x}}_{\text{it}} - {\widehat{x}}_{it|t}$=$\xi_{i}{(F)}^{+}a_{t + 1}$   \[7.60\]
  ------------------------------------------------------------------------------------------------ ----------
